...
void kruskal()
{
    ...
    int minCost=0;
    int nrEdgesAdded=0;
    int ** copyAdjMatrix = getCopyOfAdjecencyMatrix();

    while(nrEdgesAdded < nrOfVerteces-1)
    {
        edgeT minEdge = getMinimumEdgeForAdjacencyMatrix(copyAdjMatrix);
        copyAdjMatrix[minEdge.source][minEdge.destination]=MAX;
        copyAdjMatrix[minEdge.destination][minEdge.source]=MAX;
        int u,v;
        u=getParent(parent, minEdge.source);
        v=getParent(parent, minEdge.destination);
        if(uni(parent, u, v))
        {
            minCost+=minEdge.weight;
